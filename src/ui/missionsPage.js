...
import { SKILLS, MISSION_GOALS, MISSION_CREW_MAX, SEASON_LENGTH } from '../sim/config';
const GOAL_LABELS = { quick: 'Quick', regular: 'Regular' };
...
function crewPickOrder(a, b, skillId) {
    const apA = a.aptitude[skillId] ?? 1;
    const apB = b.aptitude[skillId] ?? 1;
    if (apA !== apB)
        return apB - apA; // higher aptitude first
...
const GATHER_TYPES = ['gatherFood', 'gatherResources'];
export function createMissionsPage(colony) {
    ...
    const activeCount = el.querySelector('.active-count');
    const activeList = el.querySelector('.active-list');
    const zoneCount = el.querySelector('.zone-count');
    const zoneList = el.querySelector('.zone-list');
    const recentList = el.querySelector('.recent-list');
    const openZones = new Set(); // expanded zone ids (multiple allowed)
    const home = colony.zones.find((z) => z.home); // expand the home zone (The Roost) by default
    if (home)
        openZones.add(home.id);
    const openMissions = new Set(); // active missions expanded to show their crew
    const teams = new Map(); // open mission setup -> staged crew
    const goals = new Map(); // open mission setup -> chosen goal preset
    const goalOf = (key) => goals.get(key) ?? 'regular';
    const choosing = new Map(); // setup key -> the spot's crew id being reassigned
    let activeSig = '';
    let zoneSig = '';
    let recentSig = '';
    const fills = new Map();
    // Pre-launch preview: the goal, the food it costs, and an approximate round-trip time.
    function previewHTML(type, zoneId, crewIds, goal) {
    ...
    function rankedFree(skill, excludeIds) {
    ...
    function autoFill(key, count, skill) {
        const t = new Set();
    ...
    function typeForKey(key) {
        return key === 'explore' ? 'explore' : key.split(':')[1];
    }
    function closeSetup(key) {
        teams.delete(key);
        goals.delete(key);
        choosing.delete(key);
    }
    function toggleMission(key) {
        if (teams.has(key))
            ...
            autoFill(key, 1, colony.missionSkill(typeForKey(key)));
        renderZones();
    }
    // Launch a mission and clear the committed crew from any other pending setups.
    function commitLaunch(type, zoneId, crewIds, goal) {
        if (crewIds.length === 0)
            return;
    ...
    function canRerun(m) {
        if (m.type === 'explore' && !colony.zonesRemaining)
            return false;
    ...
    function setupHTML(key, type, zoneId) {
        const t = teams.get(key) ?? new Set();
      ...
      <div class="crew-spin">
        ...
        const goalToggle = type === 'explore'
            ? ''
            : `<div class="goal-toggle">${Object.keys(MISSION_GOALS)
                .map((g) => `<button class="len-btn${goalOf(key) === g ? ' active' : ''}" data-len="${g}">${GOAL_LABELS[g]} ${Math.round(MISSION_GOALS[g] * 100)}%</button>`)
                .join('')}</div>`;
    ...
    function missionSubHTML(zoneId, type) {
    ...
    function renderZones() {
        ...
        <div class="zrow-head clickable">
          ...
          ${tag}<span class="msym zrow-chev">${zoneOpen ? 'expand_less' : 'expand_more'}</span>
            ...
            if (zoneOpen) {
                row.querySelectorAll('.msub-head').forEach((h) => h.addEventListener('click', () => toggleMission(h.dataset.key)));
            }
      ...
      ${exploreOpen ? `<div class="zrow-body">${setupHTML('explore', 'explore', null)}</div>` : ''}`;
        if (canExplore)
            erow.querySelector('.zrow-head').addEventListener('click', () => toggleMission('explore'));
        zoneList.appendChild(erow);
        wireSetups(zoneList, renderZones);
    }
    function wireSetups(container, rerender) {
        container.querySelectorAll('.setup').forEach((setupEl) => {
            const key = setupEl.dataset.key;
            const type = setupEl.dataset.mt;
            const zraw = setupEl.dataset.zone;
            ...
            setupEl.querySelectorAll('.crew-step').forEach((btn) => btn.addEventListener('click', () => {
                ...
                if (step > 0 && team.size < MISSION_CREW_MAX) {
                    ...
                    const worst = colony.crew
                        ...
                        .pop();
                    if (worst)
                        team.delete(worst.id);
                }
                choosing.delete(key);
                rerender();
            }));
            // tap a spot to open its chooser (or close it if already open)
            setupEl.querySelectorAll('.crew-card').forEach((card) => card.addEventListener('click', () => {
                const id = Number(card.dataset.spot);
                if (choosing.get(key) === id)
                    choosing.delete(key);
                else
                    choosing.set(key, id);
                rerender();
            ...
            setupEl.querySelectorAll('.chooser-row').forEach((row) => row.addEventListener('click', () => {
                const spot = choosing.get(key);
                const pick = Number(row.dataset.pick);
            ...
            setupEl.querySelectorAll('.len-btn').forEach((btn) => btn.addEventListener('click', () => {
                goals.set(key, btn.dataset.len);
                rerender();
            }));
            const launch = setupEl.querySelector('.setup-launch');
    ...
    function update() {
            ...
            if (colony.activeMissions.length === 0) {
                ...
                for (const m of colony.activeMissions) {
                    ...
                    const team = m.crewIds
                        ...
                        .filter(Boolean)
                        .map((c) => crewCardHTML(c, skill, false, colony, m.type, m.zoneId, false))
                        .join('');
            ...
            <div class="amission-body">
              ...
              <div class="amission-foot"><button class="m-recall">Recall</button></div>
                    ...
                    fills.set(m.id, {
                        ...
                        left: card.querySelector('.m-left'),
                        phase: card.querySelector('.m-phase'),
        ...
        for (const z of colony.zones) {
            ...
            if (f)
                f.textContent = `${Math.round(z.foodAbundance)}`;
            if (r)
                r.textContent = `${Math.round(z.resourceAbundance)}`;
        }
    ...
    function updateCrewStats() {
        el.querySelectorAll('.mcrew-row[data-crew], .crew-card[data-crew]').forEach((row) => {
            const c = colony.crew.find((x) => x.id === Number(row.dataset.crew));
            if (!c)
                return;
            const fill = row.querySelector('.cbarf.hp');
            const pct = row.querySelector('.hp-pct');
            if (fill && pct) {
            ...
            const skillEl = row.querySelector('.mcrew-skill');
            const lv = skillEl?.querySelector('.skill-lv');
            const xpFill = skillEl?.querySelector('.cbarf.xpf');
            if (skillEl && lv && xpFill) {
                const skillId = skillEl.dataset.skill;
    ...
    function renderRecent() {
        ...
        recentList.innerHTML = log
            ...
            .join('');
        recentList.querySelectorAll('.recent-rerun').forEach((btn) => btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const m = colony.completedMissions.find((x) => x.id === Number(btn.dataset.mid));
            if (m)
                toggleRecent(m);
        }));
...
function recentRowHTML(m, rerunnable, open) {
    let sub;
    let got;
    if (m.type === 'explore') {
        sub = `${m.crew} crew`;
...
function crewCardHTML(c, skillId, choosing, colony, type, zoneId, interactive = true) {
...
function chooserRowHTML(c, skillId, selected) {
    const skill = SKILLS[skillId];
